...
// send byte via SPI interface
// input: byte - value to send
void spi_send_byte(uint8_t byte) {
  SPDR = byte; // send data
  while (!(SPSR & (1 << SPIF))); // wait for end transfer
}

// send word via SPI interface
// input: word - value to send
void spi_send_word(uint16_t word) {
  spi_send_byte(word >> 8); // send older byte
  spi_send_byte(word);      // send younger byte
}

// send byte length command to LCD
// input: commnad_byte - command to send
...
// send word length command to LCD
// input: commnad_word - command to send
...
// send byte length data to LCD
// input: data_byte - data value to send
...
// send word length data to LCD
// input: data_word - data value to send
...
// input: colour - background colour in 332 pixel format - RRRGGGBB
void lcd_clear(uint16_t colour) { lcd_rectangle(0, 0, 175, 131, colour); }

// set pixel with custom colour
// input: x_pos - horizontal position, y_pos - vertical position
// colour - colour of pixel in 332 pixel format - RRRGGGBB
...
// after exceeding the range, the cursor returns to the beginning of the line
// input: x1,x2,y1,y2 rectangle drawing area coordinates
void lcd_set_area(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
  lcd_command_word(0xEF90);
...
// function is drawing rectangle filled with custom color
// input: x1,x2,y1,y2 rectangle co-ordinates, colour - fill colour
...
// function is drawing 1px rectangle filled with custom color
// input: x_pos, y_pos - starting positions, width - frame width
// height - frame height, colour - frame colour
...
// print 5x7px character on LCD
// input: x_pox,y_pos - position of bottom left corner character
// character - character to print, colour - colour of font
...
// print 10x14px character on LCD
// input: x_pox,y_pos - position of bottom left corner character
// character - character to print, colour - colour of font
// background - colour of background
...
// print string on LCD, auto wrapping in set once frame
// input: x_pox,y_pos - position of bottom left corner first character
// string - pointer to printed string, colour - colour of font
// background - colour of background, size - font size (0-small, 1-big)
...
// draw line on LCD
// input: x1,y1,x2,y2 line coordinates, colour - color of line
